...
const Search = () => {
	...
	const { cache, count, queue, handleCheck } = useContext(UserContext);
	...
	useEffect(() => {
		const url = search ? `/api/movie${search}&limit=20` : `/api/movie?limit=20`;
		...
		<div className="search_container">
			...
			<div className="grid wideS">
				<div className="search_filter_container">
					<select ref={sortRef} name="sort" className="search_filter-select">
					...
					>
						...
						{kinds?.map((item) => (
							<option value={item?.slug} key={item?._id + "kinds"}>
								{item?.name}
							</option>
						))}
					</select>
				...
				<div className="search_items_container">
					{movies?.Products?.length == 0 ? (
						<div className="warning_movies">Không có phim bạn muốn tìm.</div>
					) : (
						<div className="row">
			...
			<Paginating updatePS={updatePS} limit={20} count={countRef.current} />
		</div>
	);
};
